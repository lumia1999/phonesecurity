...
public class ImageViewTouchBase extends ImageView implements IDisposable {
	...
	protected boolean mCenterForbidded = false;
	...
	protected float maxZoom() {
		...
		float ratio = rect.width() >= rect.height() ? rect.width()
				/ (float) rect.height() : rect.height() / (float) rect.width();
		float width = rect.width();
		float height = rect.height();
		DisplayMetrics dm = getResources().getDisplayMetrics();
		float maxScaledW = dm.widthPixels;
		float maxScaledH = dm.heightPixels;
		// Log.e("ttt", "maxScaledW : " + maxScaledW + ",maxScaledH : "
		// + maxScaledH);
		float fw = width / maxScaledW;
		float fh = height / maxScaledH;
		float maxZoom = Math.max(fw, fh);
		if (maxZoom < 1) {
			maxZoom = maxZoom * 4 + 1;
		} else {
			maxZoom += 3.5;
	...
	protected void getProperBaseMatrix(Drawable drawable, Matrix matrix) {
		...
		if (w > viewWidth || h > viewHeight) {
			// does this 'min' have any sense?
			float widthScale = Math.min(viewWidth / w, 2.0f);
			float heightScale = Math.min(viewHeight / h, 2.0f);
			heightScale = (h * 1.0f) / viewHeight;
			float scale = Math.min(widthScale, heightScale);
			if (w < viewWidth) {
				mCenterForbidded = true;
				scale = (viewWidth * 1.0f) / w;
			}
	...
	protected void center(boolean horizontal, boolean vertical) {
		...
		if (rect.left != 0 || rect.top != 0 && !mCenterForbidded) {
