...
import com.herry.relaxreader.item.BitmapDrawableItem;
import com.herry.relaxreader.item.BitmapItem;
...
public class ImageCache {
	...
	private static final int DIST_CACHE_TYPE_INDEX = 1;
	...
	private LruCache<String, BitmapDrawableItem> mMemoryCache;
	private ImageCacheParams mCacheParams;
	private final Object mDiskCacheLock = new Object();
	private boolean mDiskCacheStarting = true;

	private HashSet<SoftReference<BitmapItem>> mReusableBitmaps;
	...
	private void init(ImageCacheParams cacheParams) {
		...
		if (mCacheParams.memoryCacheEnabled) {
			...
			if (Utils.hasHoneycomb()) {
				mReusableBitmaps = new HashSet<SoftReference<BitmapItem>>();
			}

			mMemoryCache = new LruCache<String, BitmapDrawableItem>(
					mCacheParams.memCacheSize) {
				...
				protected void entryRemoved(boolean evicted, String key,
						BitmapDrawableItem oldValue, BitmapDrawableItem newValue) {
					if (RecyclingBitmapDrawable.class.isInstance(oldValue
							.getBitmapDrawable())) {
						// The removed entry is a recycling drawable, so notify
						// it
						// that it has been removed from the memory cache
						((RecyclingBitmapDrawable) oldValue.getBitmapDrawable())
								.setIsCached(false);
					} else {
						...
						if (Utils.hasHoneycomb()) {
							...
							// inBitmap later
							mReusableBitmaps.add(new SoftReference<BitmapItem>(
									new BitmapItem(oldValue.getBitmapDrawable()
											.getBitmap(), oldValue
											.getImageType())));
						}
					}
				 ...
				 */
				@Override
				protected int sizeOf(String key, BitmapDrawableItem value) {
	 ...
	 */
	public void addBitmapToCache(String data, BitmapDrawableItem value) {
		...
		if (mMemoryCache != null) {
			if (RecyclingBitmapDrawable.class.isInstance(value
				...
				((RecyclingBitmapDrawable) value.getBitmapDrawable())
						.setIsCached(true);
			}
				...
				try {
					...
					if (snapshot == null) {
						...
						if (editor != null) {
							...
							value.getBitmapDrawable()
									.getBitmap()
									.compress(mCacheParams.compressFormat,
											mCacheParams.compressQuality, out);
							// editor.commit();
							// out.close();
							typeOut = editor
									.newOutputStream(DIST_CACHE_TYPE_INDEX);
							typeOut.write(value.getImageType().getBytes());
							editor.commit();
							out.close();
							typeOut.close();
						}
						...
						snapshot.getInputStream(DIST_CACHE_TYPE_INDEX).close();
					}
				} catch (final IOException e) {
	 ...
	 */
	public BitmapDrawableItem getBitmapFromMemCache(String data) {
		BitmapDrawableItem memValue = null;
	 ...
	 */
	public BitmapItem getBitmapFromDiskCache(String data) {
		final String key = hashKeyForDisk(data);
		...
		synchronized (mDiskCacheLock) {
			...
			if (mDiskLruCache != null) {
				...
				InputStream typeInputStream = null;
				try {
					...
					if (snapshot != null) {
						...
						typeInputStream = snapshot
								.getInputStream(DIST_CACHE_TYPE_INDEX);
						if (inputStream != null && typeInputStream != null) {
	...
	public static String readImageType(InputStream is) throws IOException {
		String content = DiskLruCache.readFully(new InputStreamReader(is));
		return content;
	}
	...
	protected Bitmap getBitmapFromReusableSet(BitmapFactory.Options options) {
		...
		if (mReusableBitmaps != null && !mReusableBitmaps.isEmpty()) {
			final Iterator<SoftReference<BitmapItem>> iterator = mReusableBitmaps
					.iterator();
			BitmapItem item;
			...
			while (iterator.hasNext()) {
				...
				if (null != item && item.getBitmap().isMutable()) {
					// Check to see it the item can be used for inBitmap
					if (canUseForInBitmap(item.getBitmap(), options)) {
	...
	private static boolean canUseForInBitmap(Bitmap candidate,
		...
		boolean ret = candidate.getWidth() == width
	 ...
	 */
	@TargetApi(12)
	public static int getBitmapSize(BitmapDrawableItem value) {
		Bitmap bitmap = value.getBitmapDrawable().getBitmap();
	...
	public static long getUsableSpace(File path) {
		long space;
		if (Utils.hasGingerbread()) {
			space = path.getUsableSpace();
		} else {
			final StatFs stats = new StatFs(path.getPath());
			space = (long) stats.getBlockSize()
