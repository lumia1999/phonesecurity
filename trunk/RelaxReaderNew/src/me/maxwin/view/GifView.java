...
public class GifView extends WebView {
	 ...
	 * 
	 */
	public interface OnGifViewClickListener {
	...
	private OnGifViewClickListener mListener;
	...
	public void loadGif(String gifPath) {
		...
		Log.d(TAG, "Loading " + gifPath + ",mImageWidth : " + mImageWidth
				+ ",mImageHeight : " + mImageHeight);
		this.loadUrl("file://" + gifPath);
		this.mGifPath = gifPath;
	}

	public void setOnGifClickListener(OnGifViewClickListener l) {
		mListener = l;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		float viewScale = this.getResources().getDimension(
				R.dimen.gif_view_scale);
		DisplayMetrics dm = this.getResources().getDisplayMetrics();
		viewScale = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1,
				dm) * viewScale;
		...
		if (parent_width < scaleImgWidth && scaleImgWidth > 0) {
			float scale = viewScale * 100 * (parent_width / origImgWidth);
			initialScale = (int) scale;
		...
		if (parent_height < scaleImgHeight && scaleImgHeight > 0) {
			...
			float scale = viewScale * 100 * (parent_height / origImgHeight);
			initialScale = (int) scale;
