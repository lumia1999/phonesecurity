...
public class TitlePageIndicator extends View implements PageIndicator {
	...
	public TitlePageIndicator(Context context, AttributeSet attrs, int defStyle) {
		...
		final float defaultTitleSelectedSize = res
				.getDimension(R.dimen.default_title_indicator_text_selected_size);
		...
		mTextSelectedSize = a.getDimension(
				R.styleable.TitlePageIndicator_selectedTextSize,
				defaultTitleSelectedSize);
	...
	protected void onDraw(Canvas canvas) {
		...
		// Log.e(TAG, "left : " + left + ",leftClip : " + leftClip + ",width : "
		// + width + ",height : " + height + ",right : " + right
		// + ",rightClip : " + rightClip);
		...
		// Log.e(TAG, "curPageBound right : " + curPageBound.right
		// + ",RightClip : " + rightClip + ",mCurrentPage : "
		// + mCurrentPage);
		...
		if (mCurrentPage > 0) {
			for (int i = mCurrentPage - 1; i >= 0; i--) {
				...
				if (bound.left < leftClip) {
					 ...
					 * @author herry
					 *         <p>
					 *         dx = bound.right+ mTitlePadding - rightBound.left
					 *         <p>
					 *         so,new bound.left set as follows:
					 *         <p>
					 *         bound.left = bound.left- dx
					 *         <p>
					 *         finally, bound.left = (int) (rightBound.left - w
					 *         - mTitlePadding)
					 *         <p>
					 */
		...
		if (mCurrentPage < countMinusOne) {
			for (int i = mCurrentPage + 1; i < count; i++) {
				...
				// Log.i(TAG, "bound.right : " + bound.right + ",rightClip : "
				// + rightClip + ",mCUrrentPage : " + mCurrentPage
				// + ",count : " + count);
		 ...
		 *         '>>>'unsigned right move
		 *         <p>
		 *         because Color is argb,it consume 4 * 8 = 32 bit
		 *         <p>
		 *         so, unsigned right move 24 bit, we can just obtain the alpha
		 *         value back
		 *         <p>
		 *         and in {@link android.graphics.Color},it has a static method
		 *         called getAlpha({@link java.lang.Integer color})
		 *         <p>
		 *         this two method will give you the same result
		 *         <p>
		 */
		int colorTextAlpha = mColorText >>> 24;
		// int colorTextAlpha = Color.alpha(mColorText);
		for (int i = 0; i < count; i++) {
			...
			if ((bound.left > left && bound.left < right)
				 ...
				 * @author herry
				 *         <P>
				 *         the following codes is omitted,because this
				 *         adjustment has done above.
				 */
				// if (i < boundsSize - 1) {
				// Rect rightBound = bounds.get(i + 1);
				// // Intersection
				...
				if (currentPage && currentSelected) {
					mPaintText.setTextSize(mTextSelectedSize);
		...
		// Log.e(TAG, "mLinePosition : " + mLinePosition);
		...
		case Triangle:
			mPath.reset();
			// make a animation
			float y = heightMinusLine - footerIndicatorLineHeight;
			y += mPageOffset * y;
			mPath.moveTo(halfWidth, y);
	...
	private ArrayList<Rect> calculateAllBounds(Paint paint) {
		...
		// Log.i(TAG, "mCurrentPage : " + mCurrentPage + ",mPageOffset : "
		// + mPageOffset);
		for (int i = 0; i < count; i++) {
