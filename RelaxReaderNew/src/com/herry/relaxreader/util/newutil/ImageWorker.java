...
public abstract class ImageWorker {
	...
	public void loadImage(Object data, ImageView imageView) {
		...
		if (RecyclingImageView.class.isInstance(imageView)) {
			// init
			RecyclingImageView riv = ((RecyclingImageView) imageView);
			riv.setExpandable(false);
			riv.setGif(false);
		}
		BitmapDrawableItem value = null;
		...
		if (value != null) {
			...
			imageView.setImageDrawable(value.getBitmapDrawable());
			if (RecyclingImageView.class.isInstance(imageView)) {
				RecyclingImageView riv = (RecyclingImageView) imageView;
				riv.setExpandable(true);
				if (TextUtils.equals(value.getImageType(),
	 ...
	 */
	protected abstract BitmapItem processBitmap(Object data);
	...
	private class BitmapWorkerTask extends
			AsyncTask<Object, Void, BitmapDrawableItem> {
		private Object data;
		 ...
		 */
		@Override
		protected BitmapDrawableItem doInBackground(Object... params) {
			if (BuildConfig.DEBUG) {
			...
			BitmapDrawableItem drawableItem = null;
			...
			if (mImageCache != null && !isCancelled()
				...
				bitmapItem = mImageCache.getBitmapFromDiskCache(dataString);
			}
			...
			if (bitmapItem == null && !isCancelled()
					&& getAttachedImageView() != null && !mExitTasksEarly) {
				bitmapItem = processBitmap(params[0]);
			}
			...
			if (bitmapItem != null) {
				if (Utils.hasHoneycomb()) {
					...
					// BitmapDrawable
					drawableItem = new BitmapDrawableItem(new BitmapDrawable(
							mResources, bitmapItem.getBitmap()),
							bitmapItem.getImageType());
				} else {
					// Running on Gingerbread or older, so wrap in a
					// RecyclingBitmapDrawable
					// which will recycle automagically
					drawableItem = new BitmapDrawableItem(
							new RecyclingBitmapDrawable(mResources,
									bitmapItem.getBitmap()),
							bitmapItem.getImageType());
				}

				if (mImageCache != null) {
					mImageCache.addBitmapToCache(dataString, drawableItem);
				}
			}
		 ...
		 */
		@Override
		protected void onPostExecute(BitmapDrawableItem value) {
			...
			if (value != null && imageView != null) {
				...
				setImageDrawable(imageView, value.getBitmapDrawable());
			}
		}

		@Override
		protected void onCancelled(BitmapDrawableItem value) {
	...
	private void setImageDrawable(ImageView imageView, Drawable drawable) {
		if (mFadeInBitmap) {
			...
			// mLoadingBitmap));
			imageView.setBackgroundDrawable(null);// remove its background
