...
public class HttpRequestBox {
	...
	private static final int DEFAULT_RETRY_TIMES = 1;// retry three times
	...
	private static final String HEADER_SUPPORT_GIF = "support_gif";
	private static final String VALUE_SUPPORT_GIF = new String("true");
	...
	private HttpRequestBox(Context ctx) {
		...
		params.setParameter(HEADER_SUPPORT_GIF, true);
	...
	public HttpResponse sendRequest(HttpRequestBase request) {
		...
		for (int i = 0; i < DEFAULT_RETRY_TIMES; i++) {
			try {
				...
				request.setHeader(HEADER_SUPPORT_GIF, VALUE_SUPPORT_GIF);
