...
public class CalllogDelDialogPreference extends DialogPreference {
	private static final String TAG = "CalllogDelDialogPreference";
	...
	public CalllogDelDialogPreference(Context context, AttributeSet attrs,
			int defStyle) {
	...
	public void onClick(DialogInterface dialog, int which) {
		...
		if (which == DialogInterface.BUTTON_POSITIVE && selectedItems != null) {
			...
			mProgressDlg.show();
			new DeleteCalllogTask(selectedItems).execute();
		}
	...
	private class DeleteCalllogTask extends AsyncTask<Void, Void, Integer> {
		private List<Integer> mDelData;

		public DeleteCalllogTask(List<Integer> data) {
			super();
			mDelData = data;
		}
